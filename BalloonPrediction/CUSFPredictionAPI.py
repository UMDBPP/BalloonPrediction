...
def request_prediction(launch_latitude,
                       launch_longitude,
                       launch_altitude=None,
                       launch_datetime='{:%Y-%m-%dT%H:%M:%SZ}'.format(
                           datetime.datetime.now()),
                       ascent_rate=average_ascent_rate,
                       burst_altitude=average_burst_altitude,
    ...
    query_request = 'http://predict.cusf.co.uk/api/v1/?launch_latitude={:f}&launch_longitude={:f}&{:s}launch_datetime={:s}&ascent_rate={:f}&burst_altitude={:f}&descent_rate={:f}'.format(
        launch_latitude, launch_longitude, launch_altitude_query, launch_datetime, ascent_rate, burst_altitude, sea_level_descent_rate)
    ...
    if query_result.status_code == 200:
        ...
        raise(Exception('{} {}: {}'.format(query_result.status_code,
                                           query_json['error']['type'], query_json['error']['description'])))
